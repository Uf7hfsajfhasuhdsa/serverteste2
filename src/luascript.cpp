...
void LuaInterface::registerFunctions()
{
	...
	//getMonsterStorageKey(cid)
	lua_register(m_luaState, "getMonsterStorageKey", LuaInterface::luaGetMonsterStorageKey);

	//getMonsterNeedStorage(cid)
	lua_register(m_luaState, "getMonsterNeedStorage", LuaInterface::luaGetMonsterNeedStorage);

	//getStorageOnKill(cid)
	lua_register(m_luaState, "getStorageOnKill", LuaInterface::luaGetStorageOnKill);
	...
	//doTileSetFlag(pos, flag, type)
    lua_register(m_luaState, "doTileSetFlag", LuaInterface::luaDoTileSetFlag);
	...
	//doPlayerChangeVisible(cid)
	lua_register(m_luaState, "doPlayerChangeVisible", LuaInterface::luaDoPlayerChangeVisible);
...
int32_t LuaInterface::luaDoPlayerChangeVisible(lua_State* L)
{
    //doPlayerChangeVisible(cid)
    ...
    if (Creature * creature = env->getCreatureByUID(popNumber(L)))
    {
        ...
        {
            player->sendTextMessage(MSG_INFO_DESCR, "Voc\EA voltou a ficar vis\EDvel.");
        ...
        else if ((condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_GAMEMASTER, -1, 0, false, GAMEMASTER_INVISIBLE)))
        {
            ...
            player->sendTextMessage(MSG_INFO_DESCR, "Voc\EA est\E1 invis\EDvel agora.");
        }
...
int32_t LuaInterface::luaDoTileSetFlag(lua_State* L)
{
    //doTileSetFlag(pos, flagId, type)
    uint32_t type = popNumber(L);
    uint32_t flagId = popNumber(L);
    ...
    tileflags_t flag = TILESTATE_NONE;
    
    
    switch(flagId)
    {
        case 1:
            flag = TILESTATE_PROTECTIONZONE;
            break;
        case 2:
            flag = TILESTATE_TRASHED;
            break;
        case 3:
            flag = TILESTATE_OPTIONALZONE;
            break;
        case 4:
            flag = TILESTATE_NOLOGOUT;
            break;
        case 5:
            flag = TILESTATE_HARDCOREZONE;
            break;
        case 6:
            flag = TILESTATE_REFRESH;
            break;
        default:
            flag = TILESTATE_NONE;
            break;
...
int32_t LuaInterface::luaGetStorageOnKill(lua_State* L)
{
    //getStorageOnKill(cid)
    ScriptEnviroment* env = getEnv();
    Creature* creature = env->getCreatureByUID(popNumber(L));
    ...
    lua_pushnumber(L, monster->getStorageOnKill());

    return 1;
}

int32_t LuaInterface::luaGetMonsterNeedStorage(lua_State* L)
{
    //getMonsterNeedStorage(cid)
    ScriptEnviroment* env = getEnv();
    Creature* creature = env->getCreatureByUID(popNumber(L));
    ...
    lua_pushnumber(L, monster->getNeedStorage());

    return 1;
}

int32_t LuaInterface::luaGetMonsterStorageKey(lua_State* L)
{
    //getMonsterStorageKey(cid)
    ScriptEnviroment* env = getEnv();
    Creature* creature = env->getCreatureByUID(popNumber(L));
    ...
    lua_pushstring(L, monster->getStorageKey().c_str());
    return 1;
}
