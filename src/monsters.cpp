...
void MonsterType::reset()
{
	...
	experience = defense = armor = lookCorpse = corpseUnique = corpseAction = conditionImmunities = damageImmunities = needStorage = 0;
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"flags"))
		{
			for(xmlNodePtr tmpNode = p->children; tmpNode; tmpNode = tmpNode->next)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"flag"))
				{
					...
					if (readXMLInteger(tmpNode, "needstorage", intValue))
						mType->needStorage = intValue;

					if (readXMLString(tmpNode, "storagekey", strValue))
						mType->storageKey = strValue;
					
					if (readXMLInteger(tmpNode, "storageonkill", intValue))
						mType->storageOnKill = intValue;
