...
Creature* Game::getCreatureByID(uint32_t id)
{
	if (id <= Player::playerAutoID) {
		...
		AutoList<Npc>::const_iterator it = Npc::autoList.find(id);
...
Player* Game::getPlayerByName(std::string s)
{
	...
	const char* pName = s.c_str();
	for(AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
	{
		if (strcasecmp(pName, it->second->getName().c_str()) == 0) {
			return it->second;
		}
	}
...
ReturnValue Game::getPlayerByNameWildcard(std::string s, Player*& player)
{
	...
	for(AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
	{
		...
		if (strcasecmp(cstr, it->second->getName().substr(0, s.length()).c_str()) != 0)
			continue;
...
PlayerVector Game::getPlayersByName(std::string s)
{
	...
	for(AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
	{
		if (strcasecmp(it->second->getName().c_str(), cstr) == 0)
	...
	const Position& movingCreaturePos, const Position& toPos, bool delay)
{
	...
	if(!Position::areInRange<1,1,0>(movingCreaturePos, player->getPosition()) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar))
	{
		...
		if(getPathToEx(player, movingCreaturePos, listDir, 0, 1, true, true))
		{
			...
			SchedulerTask* task = createSchedulerTask(SCHEDULER_MINTICKS,
...
bool Game::playerUpdateHouseWindow(uint32_t playerId, uint8_t listId, uint32_t windowTextId, const std::string& text)
{
	...
	if(!house || !house->canEditAccessList(internalListId, player) || internalWindowTextId != windowTextId || listId)
		return true;
...
{
	Player* player = getPlayerByID(playerId);
    if(!player || player->isRemoved() || player->getAccess() < 3)
        return false;
	...
	const std::string& text, uint32_t statementId)
{
	...
	if(player->getLevel() < 50) { // 50 level min to send pm.
	player->sendTextMessage(MSG_STATUS_SMALL, "Voce precisa estar level 50 para enviar PM");
	return false;
}
	...
	int32_t& healthChange, bool checkDefense, bool checkArmor, bool field/* = false*/, bool element/* = false*/)
{
	...
	{
		if(!element)
			addMagicEffect(targetPos, MAGIC_EFFECT_POFF, target->isGhost());
	...
	{
		if(!element)
			addMagicEffect(targetPos, MAGIC_EFFECT_BLOCKHIT);
	...
	addMagicEffect(targetPos, effect);
	return true;
}
