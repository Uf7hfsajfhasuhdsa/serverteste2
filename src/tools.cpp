...
bool isValidPassword(std::string text)
{
	...
	for(uint32_t size = 0; size < textLength; size++)
	{
		if(isupper(text[size]))
			maiuscula = true;
		if(islower(text[size]))
			minuscula = true;
		if(isdigit(text[size]))
			numero = true;
	}
	int sum_results_bool = maiuscula + minuscula + numero;
	if(sum_results_bool < 3)
		return false;
...
WeaponTypeNames weaponTypeNames[] = {
	...
	{"defesa", WEAPON_SHIELD},
	{"attack speed", WEAPON_FIST},
	{"wand", WEAPON_WAND},
