...
bool InstantSpell::loadFunction(const std::string& functionName)
{
	...
	else if(tmpFunctionName == "fullsearchplayer")
	{
		isAggressive = false;
		function = FullSearchPlayer;
	}
...
bool InstantSpell::FullSearchPlayer(const InstantSpell*, Creature* creature, const std::string& param)
{
	Player* player = creature->getPlayer();
	if(!player || player->isRemoved())
		return false;
	...
	int32_t dx = std::abs(player->getPosition().x - targetPlayer->getPosition().x);
	int32_t dy = std::abs(player->getPosition().y - targetPlayer->getPosition().y);
	std::stringstream ss;
	ss << targetPlayer->getName() << " " << g_game.getSearchString(player->getPosition(), targetPlayer->getPosition(), true, true) << " (DISTANCE IN SQM: " << std::max<int32_t>(dx, dy) << ").";
	player->sendTextMessage(MSG_INFO_DESCR, ss.str().c_str());
