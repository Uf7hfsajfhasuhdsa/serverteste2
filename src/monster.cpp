...
void Monster::onCreatureAppear(const Creature* creature)
{
	...
	if(creature == this)
	{
		...
		if (mType->needStorage > 0 && mType->storageKey != "0")
			storageMonstersCheck();
...
bool Monster::isOpponent(const Creature* creature)
{
	if (mType->needStorage > 0 && mType->storageKey != "0") {
		std::string strValue = "";
		creature->getStorage(mType->storageKey, strValue);

		if (std::stoi(strValue) == mType->needStorage) {
			return true;
		}
		else {
			return false;
...
void Monster::storageMonstersCheck()
{
	...
	{
		std::string strValue = "";
		(*it)->getStorage(mType->storageKey, strValue);
		if ((*it)->getHealth() <= 0 || !canSee((*it)->getPosition()) || strValue != mType->storageKey)
		{
			(*it)->unRef();
			it = targetList.erase(it);
		}
	...
	Scheduler::getInstance().addEvent(createSchedulerTask(500, boost::bind(&Monster::storageMonstersCheck, this)));
}
