...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_NOTREQUIREDLEVELTOUSERUNE:
			sendCancel("You do not have the required ninjutsu level to use this rune.");
			break;
		...
		case RET_NOTENOUGHMAGICLEVEL:
			sendCancel("You do not have enough ninjutsu.");
			break;

		case RET_NOTENOUGHMANA:
			sendCancel("You do not have enough chakra.");
			break;
		...
		case RET_YOUNEEDAMAGICITEMTOCASTSPELL:
			sendCancel("You need a ninjutsu item to cast this jutsu.");
			break;
...
void Player::addManaSpent(uint64_t amount, bool useMultiplier/* = true*/)
{
	...
	if(magLevel > 0 && currReqMana > nextReqMana) //player has reached max ninjutsu
		return;
	...
	while(manaSpent + amount >= nextReqMana)
	{
		...
		s.str("");
		s << "You advanced to ninjutsu level " << ++magLevel << ".";
...
bool Player::onDeath()
{
	...
	if(skillLoss)
	{
		...
		for(int16_t i = 0; i < 7; ++i) // for each skill
		{
			...
			skills[i][SKILL_PERCENT] = Player::getPercentLevel(skills[i][SKILL_TRIES], vocation->getReqSkillTries(i, skills[i][SKILL_LEVEL] + 1));
		}
...
Item* Player::createCorpse(DeathList deathList)
{
	...
	if(deathList.size() > 1 && g_config.getBool(ConfigManager::MULTIPLE_NAME))
	{
		...
		else if(strcasecmp(deathList[0].getKillerName().c_str(), deathList[1].getKillerName().c_str()) != 0)
			ss << " and by " << deathList[1].getKillerName();
	}
...
int32_t SpeedAttack;
  ...
  if(SpeedAttack < 10)
    SpeedAttack = 1500;
 else if(SpeedAttack >= 10 && SpeedAttack < 40)
    SpeedAttack = 1200;
 else if(SpeedAttack >= 40 && SpeedAttack < 80)
    SpeedAttack = 1100;
 else if(SpeedAttack >= 80 && SpeedAttack < 100)
    SpeedAttack = 1000;
		...
		case MANAGER_ACCOUNT:
		{
			...
			else if(talkState[4])
			{
				...
				if(tmp.length() < 6)
					...
					msg << "Your password needs to have a uppercase letter, a lowercase letter, and numbers. Sua senha precisa ter no minimo 1 letra {maiuscula}, e um numero. Ex: {Passw2}";
				else
				{
					talkState[4] = false;
					talkState[5] = true;
		...
		case MANAGER_NEW:
		{
			...
			else if(checkText(text, "yes") && talkState[3])
			{
				if(g_config.getBool(ConfigManager::GENERATE_ACCOUNT_NUMBER))
				{
					...
					uint32_t id = (uint32_t)IOLoginData::getInstance()->createAccount(managerChar, managerString, client->getIP());
					if(id)
					{
