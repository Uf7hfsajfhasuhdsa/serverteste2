...
local function greetCallback(cid)
	local msg = 'Olá '.. getCreatureName(cid) .."o que deseja ? {Ajuda} {Batalhar}"
...
function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, 'Batalhar') or msgcontains(msg, 'batalhar')) then
		selfSay('Pronto ? {sim}.', cid)
		talkState[talkUser] = 2
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 2) then
	if getPlayerStorageValue(cid, t.storage1) < os.time()  then
		doTeleportThing(cid, t.pos)
		setPlayerStorageValue(cid, t.storage1, os.time() + t.tempo1 * 60 )
		selfSay('Ok', cid)
	else
		doPlayerPopupFYI(cid, "[ Hokage ] estou cansado ! Volte em ".. getPlayerStorageValue(cid, t.storage1) - os.time() .." segundos.")
	end
	talkState[talkUser] = 0
