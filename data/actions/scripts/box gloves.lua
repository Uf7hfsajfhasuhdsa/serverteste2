...
{itemId = 2648, count = 1, chance = 10}, -- ITEM, QUANTIDADE, CHANCE
{itemId = 2372, count = 1, chance = 10},
{itemId = 2373, count = 1, chance = 10},

{itemId = 2516, count = 1, chance = 10},
{itemId = 2388, count = 1, chance = 5},
{itemId = 7895, count = 1, chance = 5},
{itemId = 11433, count = 1, chance = 2},
{itemId = 2387, count = 1, chance = 1},
...
local config = {
	...
	exhaustionSeconds = 60, -- exausted em segundos
	storageUse = 43531, -- storage usado.

}

if(exhaustion.check(cid, config.storageUse)) then
if (exhaustion.get(cid, config.storageUse) >= 60) then
doPlayerSendCancel(cid, "Você somente poderá abrir uma nova Gloves Box em " .. math.floor(exhaustion.get(cid, config.storageUse) / 60 + 1) .." minutos.") 
end
if (exhaustion.get(cid, config.storageUse) <= 60) then
