...
local speed = 960 -- velocidade do player ao usar o dash (vai de 0 a mil)
	...
	if isCreature(cid) then 
		...
		if isWalkable(poslook, false, false, pzprotect) then
			if not isCreature(getThingfromPos(poslook).uid) then
				...
				doSendMagicEffect(getPlayerPosition(cid), 97)
			else
...
local time = 5
	...
	for i = 0, distance do
		addEvent(onWalk, (1001- math.min(speed, 2000)) *i, cid)
	end
return true
else
