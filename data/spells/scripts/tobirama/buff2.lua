...
local effect = {262} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
local health = 5000 -- A cada 1 segundo quantos aumentar de vida
...
function magicEffect2001(tempo2,tempo3,cid)
...
local position217 = {x=getPlayerPosition(cid).x+1, y=getPlayerPosition(cid).y, z=getPlayerPosition(cid).z}

doSendMagicEffect(position217, 262)  
...
addEvent(magicEffect2001, tempo2, tempo2, tempo*1000, cid)
...
doCreatureSay(cid, "Suiton Mizu", TALKTYPE_MONSTER)
    
doSendMagicEffect(position127, 176)
