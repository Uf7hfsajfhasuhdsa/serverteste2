...
function magicEffect95(tempo2,tempo3,cid)
				...
				if #summons > 0 then
					for k = 1, #summons do
						if isInArray(names, getCreatureName(summons[k])) then
							...
							local positions = {x = pos.x + 0, y = pos.y, z = pos.z}
...
local position128 = {x=getPlayerPosition(cid).x+0, y=getPlayerPosition(cid).y, z=getPlayerPosition(cid).z}
...
addEvent(magicEffect95, tempo2, tempo2, tempo*1000, cid)
...
doSendMagicEffect(position128, 122)
