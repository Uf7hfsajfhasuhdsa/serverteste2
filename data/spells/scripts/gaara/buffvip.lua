...
local effect = {436} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
function magicEffect900(tempo2,tempo3,cid)
...
local position = {x=getPlayerPosition(cid).x+1, y=getPlayerPosition(cid).y+0, z=getPlayerPosition(cid).z}
...
local position900 = {x=getPlayerPosition(cid).x+1, y=getPlayerPosition(cid).y, z=getPlayerPosition(cid).z}
...
addEvent(magicEffect900, tempo2, tempo2, tempo*1000, cid)
...
doSendMagicEffect(position900, 134)
